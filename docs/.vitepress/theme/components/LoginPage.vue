    ...
    <div class="login-card">
      ...
      <template v-if="mode === 'forgot'">
        ...
        <template v-if="forgotStep === 1">
          <p class="forgot-desc">请输入注册时使用的邮箱，我们将向该邮箱发送验证码。</p>
          <form class="form" @submit.prevent="sendForgotCode">
            <div class="field">
              <label>邮箱地址</label>
              <input v-model="forgot.email" type="email" placeholder="请输入邮箱" autocomplete="email" required />
            </div>
            <p v-if="forgotError" class="error-msg">{{ forgotError }}</p>
            <button type="submit" class="submit-btn" :disabled="forgotCountdown > 0">
              {{ forgotCountdown > 0 ? `已发送，${forgotCountdown}s 后可重新发送` : '发送验证码' }}
            </button>
          </form>
        </template>
        ...
        <template v-else-if="forgotStep === 2">
          <p class="forgot-desc">验证码已发送至 <strong>{{ forgot.email }}</strong>，请查收。</p>
          <form class="form" @submit.prevent="handleResetPassword">
            <div class="field">
              <label>验证码</label>
              <div class="code-row">
                <input v-model="forgot.code" type="text" placeholder="请输入验证码" maxlength="6" required />
            ...
            <div class="field">
              <label>新密码</label>
              <input v-model="forgot.password" type="password" placeholder="请设置新密码（至少6位）" autocomplete="new-password" required minlength="6" />
            </div>
            <div class="field">
              <label>确认新密码</label>
              <input v-model="forgot.confirm" type="password" placeholder="请再次输入新密码" autocomplete="new-password" required />
            </div>
            <p v-if="forgotError" class="error-msg">{{ forgotError }}</p>
        ...
        <template v-else-if="forgotStep === 3">
          <div class="reset-success">
            ...
            <p class="success-sub">请使用新密码登录</p>
      ...
      <template v-else>
        ...
        <div class="method-switch">
          <button :class="{ active: method === 'account' }" @click="switchMethod('account')">账号密码</button>
          <button :class="{ active: method === 'phone' }" @click="switchMethod('phone')">手机号</button>
        </div>
        ...
        <form v-if="mode === 'login' && method === 'account'" class="form" @submit.prevent="handleLogin">
          <div class="field">
            <label>用户名</label>
            <input v-model="login.username" type="text" placeholder="请输入用户名" autocomplete="username" required />
          </div>
          <div class="field">
            <label>密码</label>
            <input v-model="login.password" type="password" placeholder="请输入密码" autocomplete="current-password" required />
          </div>
          <div class="field-row">
            ...
            <button type="button" class="forgot-link" @click="openForgot">忘记密码？</button>
          </div>
        ...
        <form v-else-if="mode === 'login' && method === 'phone'" class="form" @submit.prevent="handlePhoneLogin">
          <div class="field">
            <label>手机号</label>
            <input v-model="phoneLogin.phone" type="tel" placeholder="请输入手机号" maxlength="11" required />
          </div>
          <div class="field">
            <label>验证码</label>
            <div class="code-row">
              <input v-model="phoneLogin.code" type="text" placeholder="请输入验证码" maxlength="6" required />
        ...
        <form v-else-if="mode === 'register' && method === 'account'" class="form" @submit.prevent="handleRegister">
          <div class="field">
            <label>用户名</label>
            <input v-model="reg.username" type="text" placeholder="请输入用户名" autocomplete="username" required />
          </div>
          <div class="field">
            <label>邮箱</label>
            <input v-model="reg.email" type="email" placeholder="请输入邮箱" autocomplete="email" required />
          </div>
          <div class="field">
            <label>密码</label>
            <input v-model="reg.password" type="password" placeholder="请设置密码（至少6位）" autocomplete="new-password" required minlength="6" />
          </div>
          <div class="field">
            <label>确认密码</label>
            <input v-model="reg.confirm" type="password" placeholder="请再次输入密码" autocomplete="new-password" required />
          </div>
        ...
        <form v-else-if="mode === 'register' && method === 'phone'" class="form" @submit.prevent="handlePhoneRegister">
          <div class="field">
            <label>手机号</label>
            <input v-model="phoneReg.phone" type="tel" placeholder="请输入手机号" maxlength="11" required />
          </div>
          <div class="field">
            <label>验证码</label>
            <div class="code-row">
              <input v-model="phoneReg.code" type="text" placeholder="请输入验证码" maxlength="6" required />
          ...
          <div class="field">
            <label>设置密码</label>
            <input v-model="phoneReg.password" type="password" placeholder="请设置密码（至少6位）" autocomplete="new-password" required minlength="6" />
          </div>
          <div class="field">
            <label>确认密码</label>
            <input v-model="phoneReg.confirm" type="password" placeholder="请再次输入密码" autocomplete="new-password" required />
          </div>
...
function sendForgotCode() {
  ...
  // 模拟发送
  forgotMessage.value = `验证码已发送至 ${forgot.email}（模拟）`
...
function sendCode(scene) {
  ...
  if (!isValidPhone(phone)) {
    if (scene === 'login') phoneLoginError.value = '请输入正确的手机号'
    else phoneRegError.value = '请输入正确的手机号'
    return
  }
  if (scene === 'login') phoneLoginError.value = ''
  else phoneRegError.value = ''
  message.value = `验证码已发送至 ${phone}（模拟）`
...
function handleLogin() {
  message.value = `欢迎回来，${login.username}！`
}

function handlePhoneLogin() {
  phoneLoginError.value = ''
  if (!isValidPhone(phoneLogin.phone)) {
    phoneLoginError.value = '请输入正确的手机号'
    return
  }
  message.value = `欢迎回来，${phoneLogin.phone}！`
}
...
function handleRegister() {
  ...
  message.value = `注册成功，欢迎 ${reg.username}！`
}

function handlePhoneRegister() {
  phoneRegError.value = ''
  if (!isValidPhone(phoneReg.phone)) {
    phoneRegError.value = '请输入正确的手机号'
    return
  }
  if (phoneReg.password !== phoneReg.confirm) {
    phoneRegError.value = '两次输入的密码不一致'
    return
  }
  message.value = `注册成功，欢迎使用 ${phoneReg.phone}！`
}
</script>
