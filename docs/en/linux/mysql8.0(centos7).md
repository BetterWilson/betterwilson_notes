## MySQL 8.0 Installation (CentOS 7)

## 0 Pre-installation Preparation

Uninstall any leftover `mariadb`, then install the `libaio-devel` dependency library to avoid errors later.
...
From the official website https://dev.mysql.com/downloads/mysql/ download it to your local machine and upload it to the cloud server.

You can also wget directly on the server.
...
### 2.2 Create a Data Storage Directory and Grant Permissions
...
### 2.3 Extract the Installation Package
...
### 2.4 Create a Symbolic Link from the Installation Directory to `/usr/local/mysql`
...
### 2.5 Add `/usr/local/mysql` to the Environment Variables
...
**Note: if `mysql -V` does not return a version number, try deleting the symbolic link and recreating it.**

### 2.6 Create the MySQL Configuration File
...
**Note: as long as there are no `ERROR`-level errors during initialization, the initialization was successful.**

### 2.8 Startup Test

You can now start the MySQL service from any directory by running the following command.
...
### 2.9 Manage the MySQL Service with `systemctl`
...
After a successful initialization, MySQL creates a local `root` user by default, and this user has no password, so we need to add a password for this user. Moreover, this root user can usually only be used when an administrator logs into the server; your application should create a separate user with normal privileges. So we need to do two things: set a password for the root user, and from now on use this user to manage other users and permissions.
