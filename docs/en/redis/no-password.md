## Transferring data between servers without a password

Here we demonstrate transferring `redis` and its configuration files between servers without a password.
...
After this, the two machines can transfer data between each other without a password.

The `rsync` tool is used here. `rsync` (remote synchronize) is a remote data synchronization tool for Linux/Unix.
