...
> **用数字理解**：$W_3 W_2 W_1$ 三个矩阵相乘结果还是**一个矩阵**。令 $W_{total} = W_3 W_2 W_1$，则 $\hat{y} = W_{total} x + b$，和直接做线性回归没区别。**100 层的纯线性网络 = 1 层的线性网络**。激活函数打破了这种"坍缩"，让网络能拟合曲线、分类等复杂模式。
...
> **观察规律**：
>
> - 同一列（同一个输入）看：$w_{21}$ 梯度是 $w_{11}$ 的 1.5 倍（因为 $x_2=3$ 是 $x_1=2$ 的 1.5 倍）
> - 同一行（同一个神经元）看：神经元 2 的梯度都比神经元 1 大（因为 $\delta^{(1)}_2=-0.9$ 比 $\delta^{(1)}_1=-0.6$ 更大）
...
PyTorch 支持两种类型的微分：**数值微分（Numerical Differentiation）**和**符号微分（Symbolic Differentiation）**：

- 数值微分（近似求导）：数值微分是使用近似方法计算导数的一种技术。在数值微分中，通过在函数的某个点附近进行有限差分来估计导数。PyTorch 提供了数值微分的功能，可以使用 torch.autograd.grad 函数来计算函数的数值导数。**数值微分的优点是简单易用，适用于任何函数，但是计算精度可能受到数值误差的影响**

- 符号微分（代数求导）：符号微分是使用代数方法计算导数的一种技术。在符号微分中，通过对函数的符号表达式进行求导来得到导数的符号表达式。**PyTorch** **在计算图构建过程中使用符号微分来自动计算梯度**。通过在计算图中跟踪操作的导数计算，PyTorch 可以在反向传播过程中自动计算梯度。符号微分的优点是精确性和效率，但对于复杂的函数，求导的符号表达式可能会变得复杂

PyTorch 的自动微分（Automatic Differentiation）是**反向模式自动微分（Reverse Mode AD）**，它既不是纯数值微分也不是纯符号微分：它在每个基本运算（如加法、乘法、ReLU）上应用已知的导数规则（类似符号微分），但通过计算图把各运算的局部导数用链式法则串联起来（而非展开为完整符号表达式），从而实现高效、精确的梯度计算。
...
总结来说，数值微分是通过近似方法计算导数，适用于任何函数，但精度可能受到数值误差的影响。符号微分是通过代数方法计算导数，提供了精确性，但对于复杂函数，求导的符号表达式可能会变得复杂。PyTorch 的自动微分利用计算图构建和反向传播来实现梯度计算，兼具精确性和效率。

在某些情况下，PyTorch 可能使用数值微分来验证梯度计算的正确性。这通常**发生在用户自定义的操作或函数上**，当无法通过符号微分计算导数时，PyTorch 可能会使用数值微分进行梯度的数值验证。然而，这种情况是为了验证梯度计算的正确性，而不是作为主要的梯度计算方法。
...
> **核心原因**：没有激活函数（或激活函数是线性的）时，多层网络等价于单层网络（参见上文"纯线性网络的退化结论"）。激活函数引入了**非线性**，让网络能够拟合任意复杂度的函数。打个比方：线性网络只能画直线，加激活函数后就能画曲线了。
...
- **ReLU**
  ...
  ReLU 单元比较脆弱并且可能"死掉"，而且是不可逆的，因此导致了数据多样化的丢失。通过合理设置学习率，会降低神经元"死掉"的概率。(死掉是指在 $z$ 为负时输出为零，梯度也为零，一旦某个神经元的所有输入都让它输出负数，它就再也学不回来了）
  ...
  **Leaky ReLU** 其中 ε 是很小的正数斜率值，比如 0.01，Leaky ReLU 非线性函数图像如下图所示。这样做目的是使负轴信息不会全部丢失，解决了 **ReLU** 神经元"**死掉**"的问题。更进一步的方法是 PReLU，即把 ε 当做每个神经元中的一个参数，是可以通过梯度下降求解的。
...
但这个处理对于在[-1,1]之间的梯度变化不大的激活函数，效果不仅不好，反而更差。比如sigmoid函数，s函数在[-1,1]之间几乎是线性，BN变换后就没有达到非线性变换的目的；而对于relu，效果会更差，因为会有一半的置零。总之换言之，**减均值除方差操作后可能会削弱网络的性能**

因此，必须进行一些转换才能将分布从0移开。使用缩放因子$\gamma$和移位因子$\beta$来执行此操作 。下面就是加了缩放加移位后的BN完整算法。

![image-20260712142140297](assets/image-20260712142140297.png)
...
与标准 Dropout 相比，**AlphaDropout** **在将一些输出设置为零的同时，还会对剩余的输出进行缩放，以保持网络权重的期望值不变**。（保持输出的均值和方差不变）

这种缩放可以保持网络的动态范围，有助于训练过程的稳定性。

AlphaDropout 通常用于带有 SELU 激活函数的网络（配合 SELU 的自归一化特性），因为这些激活函数在负输入时不饱和，Dropout 可能会导致这些神经元的权重更新不足。
...
- Batch Size=1，梯度变来变去，非常不准确，网络很难收敛（不存在）
- Batch Size增大，梯度变准
...
| **优化器状态**           | SGD：≈ 参数量 × 4 Byte；Adam：≈ 参数量 × 8 Byte        | ✅ 训练需要，推理不需要                   |
...
| 优化器             | 保存的状态              | 倍数（相对参数量） | 100MB 模型的实际占用 |
| ------------------ | ----------------------- | ------------------ | -------------------- |
| **SGD（无动量）**  | 无额外状态              | 1×                 | 100 MB               |
| **SGD + Momentum** | 动量缓冲区 $v$          | 2×                 | 200 MB               |
| **RMSprop**        | 平方梯度移动平均        | 2×                 | 200 MB               |
| **Adam / AdamW**   | 一阶矩 $m$ + 二阶矩 $v$ | 3×                 | 300 MB               |
...
| ------------------------------ | -------------------------------------------- | ------------ |
| Embedding 输出                 | `[32, 512, 768]`                             | ~50 MB       |
| 每层 Self-Attention 的 Q, K, V | `[32, 512, 768]` × 3                         | ~150 MB / 层 |
| 每层 Attention Score           | `[32, 12, 512, 512]`                         | ~150 MB / 层 |
| 每层 FFN 中间输出              | `[32, 512, 3072]`                            | ~200 MB / 层 |
| 12 层 Transformer 的激活值总计 | —                                            | **~5~10 GB** |

而这样一个 BERT-base 模型的参数只有 ~110M ≈ **440 MB**！激活值轻松超过参数的 **10~20 倍**。
...
| **ReLU 替代为 in-place 操作**            | `nn.ReLU(inplace=True)` 原地修改，不额外分配内存 | 影响计算图（会丢失原始输入），需谨慎使用   |
...
PyTorch 有两种保存策略（由 `ctx.save_for_backward` 控制）：

| 策略         | 保存什么               | 内存占用                                      | 做法                      |
| ------------ | ---------------------- | --------------------------------------------- | ------------------------- |
| 保存输入 `x` | 整个原始张量           | 和输入一样大                                  | backward 时重新判断 x > 0 |
| 保存 `mask`  | 布尔掩码（True/False） | 输入的 1/4（bool = 1 Byte vs float = 4 Byte） | backward 时直接用 mask    |

PyTorch 默认**保存输入 `x`**（出于通用性考虑），但对于超大张量，只保存 mask 可以节省不少内存。
...
PyTorch 后台构建的计算图（简化）：
x ──→ [MmBackward0] ──→ [AddBackward0] ──→ [ReluBackward0] ──→ [MmBackward0] ──→ [AddBackward0] ──→ [MeanBackward0] ──→ loss
      (矩阵乘法)        (加法/偏置)         (ReLU)              (矩阵乘法)         (加法/偏置)         (均值)
```
...
class SimpleCNN(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(3, 64, 3, padding=1)    # 3→64 通道
        self.conv2 = nn.Conv2d(64, 128, 3, padding=1)   # 64→128 通道
        self.fc = nn.Linear(128 * 8 * 8, 10)            # 全连接 → 10 分类
    ...
    def forward(self, x):          # x: [batch, 3, 32, 32]
        ...
        x = self.relu(self.conv2(x))   # → [batch, 128, 16, 16]
...
| 激活值（各层输出之和） | batch × 各层元素数 × 4 Byte | **~200 MB** |
| **训练总计**           |                             | **~202 MB** |

参数 + 梯度 + 优化器只有 **2.4 MB**，但激活值占了 **~200 MB**！激活值比参数大 **~330 倍**。
