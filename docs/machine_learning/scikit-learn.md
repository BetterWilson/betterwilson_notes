...
补充：独热码(one-hot)是一种**一位有效编码**，其核心特征是：在一组编码中，**任意时刻只有一个****二进制位为** 1**，其余所有位均为** 0。这种编码方式不依赖数值大小表示信息，而是通过 “哪一位为 1” 来唯一标识一个状态或类别，因此也被称为 “独一热码” 或 “一位有效码”。
...
`StandardScaler`将特征转换为**均值为** 0**、标准差为** 1的分布，保留数据的相对离散程度，数据的分布依然是原有的分布
