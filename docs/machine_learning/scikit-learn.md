...
补充：独热码(one-hot)是一种**一位有效编码**，其核心特征是：在一组编码中，**任意时刻只有一个二进制位为1，其余所有位均为0**。这种编码方式不依赖数值大小表示信息，而是通过 “哪一位为 1” 来唯一标识一个状态或类别，因此也被称为 “独一热码” 或 “一位有效码”。
...
$$
IDF(t)=\log(\frac{\text{总文档数}}{\text{包含词t的文档数}+1})
$$
